...
// 1!A:A
{WORKBOOK_PREFIX}?{DOLLAR}?[{UNICODE_LETTER_RANGE}]+[:]{WORKBOOK_PREFIX}?{DOLLAR}?[{UNICODE_LETTER_RANGE}]+
        { if (parser.isValidCellRange(yytext)) { yytext = parser.convertYYTEXTtoRangeRefObject(yytext); return 'RANGEREF'; } else { this.reject(); } }
// 1!2:2
{WORKBOOK_PREFIX}?{DOLLAR}?[0-9]+[:]{WORKBOOK_PREFIX}?{DOLLAR}?[0-9]+
        { if (parser.isValidCellRange(yytext)) { yytext = parser.convertYYTEXTtoRangeRefObject(yytext); return 'RANGEREF'; } else { this.reject(); } }


// ORDER HERE IS FUNDAMNETAL!

// REVENUE[2015] : COSTS[2019] :
...
{SOURCE}{WORKBOOK_STRING_PREFIX}{DOLLAR}?[{UNICODE_LETTER_RANGE}]+{DOLLAR}?[0-9]+
		{
            console.log("Eating dead parser", yytext);
            if (parser.isValidCellRef(yytext)) {
                yytext = parser.convertYYTEXTtoCellRefObject(yytext);
                return 'CELLREF';
            }
...
start_parsing
    ...
    | do_the_work error                                                         //%build=userinput
        {
...
//        }
    ...
    | CELLREF_PARSE_MODE cell_reference                                         //%build=userinput
        {
            $$ = $cell_reference;
        }

    | CELLREF_PARSE_MODE error                                                  //%build=userinput
        {
    ...
    | RANGEREF_PARSE_MODE any_range_reference                                   //%build=userinput
        {
            /*
    ...
    | RANGEREF_PARSE_MODE error                                                  //%build=userinput
        {
            rv = yy.parser.parseError("Expected a cell range reference.", {
                text: $error,
                token: $error,
    ...
    | VALUE_PARSE_MODE accept_direct_numeric_value                               //%build=userinput
        {
            /*
             * TODO:
             *
    ...
    | VALUE_PARSE_MODE accept_direct_string_value                                 //%build=userinput
        {
            /*
             * TODO:
             *
    ...
    | VALUE_PARSE_MODE accept_direct_boolean_value                                //%build=userinput
        {
            /*
             * TODO:
             *
    ...
    | VALUE_PARSE_MODE error                                                      //%build=userinput
        {
...
boolean_expression
    ...
    | boolean_expression[first] NOT_EQUAL boolean_expression[last]
        {
    ...
    | boolean_expression[first] NOT_IDENTICAL boolean_expression[last]
        {
            // WARNING: see the Excel note below at IS_EQUAL: we don't really have a type-agnostic compare available!
            $first = coerceValueToNumeric($first);
            $last = coerceValueToNumeric($last);
    ...
    | boolean_expression[first] ALMOST_EQUAL boolean_expression[last]
        {
    ...
    | boolean_expression[first] IS_IDENTICAL boolean_expression[last]
        {
            // WARNING: see the Excel note below at IS_EQUAL: we don't really have a type-agnostic compare available!
            $first = coerceValueToNumeric($first);
            $last = coerceValueToNumeric($last);
    ...
    | boolean_expression[first] GREATER_OR_EQUAL boolean_expression[last]
        {
            $first = coerceValueToNumeric($first);
            $last = coerceValueToNumeric($last);
    ...
    | boolean_expression[first] LESS_OR_EQUAL boolean_expression[last]
        {
            $first = coerceValueToNumeric($first);
            $last = coerceValueToNumeric($last);
            // Excel: `=FALSE>1.0e300` gives TRUE: we do not emulate that behaviour here.
    ...
    | boolean_expression[first] '>'[greater_than] boolean_expression[last]
        {
            $first = coerceValueToNumeric($first);
            $last = coerceValueToNumeric($last);
            // Excel: `=FALSE>1.0e300` gives TRUE: we do not emulate that behaviour here.
    ...
    | boolean_expression[first] '<'[less_than] boolean_expression[last]
        {
            $first = coerceValueToNumeric($first);
            $last = coerceValueToNumeric($last);
            // Excel: `=FALSE>1.0e300` gives TRUE: we do not emulate that behaviour here.
    ...
    | boolean_expression[first] '='[equals] boolean_expression[last]
        {
    ...
    | boolean_expression[first] IS_EQUAL boolean_expression[last]
        {
            /*
 ...
 */
      boolean_expression[first] STRING_CONCATENATION_OPERATOR boolean_expression[last]
        {
 ...
 */
      boolean_expression[first] '+'[add] boolean_expression[last]
        {
            $first = coerceValueToNumeric($first);
            $last = coerceValueToNumeric($last);
    ...
    | boolean_expression[first] '-'[subtract] boolean_expression[last]
        {
            $first = coerceValueToNumeric($first);
            $last = coerceValueToNumeric($last);
            // Excel: `=FALSE>1.0e300` gives TRUE: we do not emulate that behaviour here.
 ...
 */
      boolean_expression[first] '*'[multiply] boolean_expression[last]
        {
            $first = coerceValueToNumeric($first);
            $last = coerceValueToNumeric($last);
    ...
    | boolean_expression[first] '/'[divide] boolean_expression[last]
        {
            $first = coerceValueToNumeric($first);
            $last = coerceValueToNumeric($last);
            $$ = ($first / $last);
    ...
    | boolean_expression[first] MODULO_OPERATOR boolean_expression[last]
        {
            $first = coerceValueToNumeric($first);
            $last = coerceValueToNumeric($last);
            $$ = ($first % $last);
 ...
 * >     = 2**3**2 ~ 2**(3**2) = 512
 */
      boolean_expression[first] '^'[power] boolean_expression[last]
        {
            $first = coerceValueToNumeric($first);
            $last = coerceValueToNumeric($last);
            $$ = Math.pow($first, $last);
    ...
    | SQRT_OPERATOR boolean_expression
        {
            $boolean_expression = coerceValueToNumeric($boolean_expression);
            $$ = Math.sqrt($boolean_expression);
        }

    | boolean_expression SQUARE_OPERATOR
        {
            $boolean_expression = coerceValueToNumeric($boolean_expression);
            $$ = Math.pow($boolean_expression, 2);
        }

    | boolean_expression CUBE_OPERATOR
        {
            $boolean_expression = coerceValueToNumeric($boolean_expression);
            $$ = Math.pow($boolean_expression, 3);
        }

    | boolean_expression '?'[TOPS20_help_request]
        {
...
data_source
    ...
    | multivalued_data_source '.'[dot] json_filter_expression                   //%build=advanced_language
        {
 ...
 *
 *      math_expression -> boolean_expression -> boolean_expression
 *          -> boolean_expression -> boolean_expression
 *          -> ... -> boolean_expression -> value -> number -> INTEGER_NUMBER
 *
 ...
 *     - reduce by rule: boolean_expression -> string
 *
 * Tread carefully...
 */
remote_address
...
//        {
            ...
            if (!($RANGEREF instanceof parser.RangeRefRepresentation)) {
                range = new parser.RangeRefRepresentation($RANGEREF);
            } else {
                range = $RANGEREF;
            }
            $$ = range;
