...
"""
Module for matrix implementation.
matrix = Mat( ({'a','b'}, {'@', '#', '?'}), {('a','@'):1, ('a','#'):2, ('a','?'):3, ('b','@'):10, ('b','#'):20, ('b','?'):30} )
matrix_as_list = [ [1, 2, 3], [10, 20, 30] ]
...
def getitem(M, k):
    ...
    if tuple == type(k):  # getting number by pair of indices
        ...
        if k in M.D[1]:
            return Vec(M.D[0], {i: M.f.get((i,k), 0) for i in M.D[0]} )  # return column labeled with k
        else:  # in M.D[0]
            return Vec(M.D[1], {i: M.f.get((k,i), 0) for i in M.D[1]} )  # return row labeled with k
...
def equal(A, B):
    ...
    """
    assert A.D == B.D
    for i in set(A.f) | set(B.f):
        # if getitem(A, i) != getitem(B, i):  # equivalent to A[i], B[i] thanks to oper overloading and tuple unpacking
        #     return False
        if A[i] != B[i]:
            return False
    return True
...
def scalar_mul(M, x):
    ...
    True
    """
    return Mat(M.D, { i:x*fi for i, fi in M.f.items() })
...
def transpose(M):
    ...
    True
    """
    return Mat( (M.D[1],M.D[0]), { (i[1],i[0]):fi for i,fi in M.f.items() } )
