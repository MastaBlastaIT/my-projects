...
%{
    ...
    */
    ...
    %type <formals> formals_nonempty
    %type <formal> formal
    %type <cases> cases
    %type <case_> case
    %type <expressions> expressions
    %type <expressions> expressions_nonempty
    ...
    */
    ...
    feature: OBJECTID '(' formals ')' ':' TYPEID '{' expression '}'   /* method def */
      { $$ = method(idtable.add_string($1->get_string()), $3, $6, $8); }
    | OBJECTID ':' TYPEID ASSIGN expression  /* attribute def */
      { $$ = attr(idtable.add_string($1->get_string()), $3, $5); }
    | OBJECTID ':' TYPEID 
      { $$ = attr(idtable.add_string($1->get_string()), $3, no_expr()); }
    ;
    ...
    formal: OBJECTID ':' TYPEID
      { $$ = formal(idtable.add_string($1->get_string()), $3); }
    ;
    ...
    expression: OBJECTID ASSIGN expression 
      { $$ = assign(idtable.add_string($1->get_string()), $3); }
    | expression '@' TYPEID '.' OBJECTID '(' expressions ')'
      { 
        $$ = static_dispatch($1, $3, idtable.add_string($5->get_string()), $7); 
      }
    | expression '.' OBJECTID '(' expressions ')'
      { $$ = dispatch($1, idtable.add_string($3->get_string()), $5); }
    ...
    | '(' expression ')'
      { $$ = $2; }
    | INT_CONST
      { @$ = @1; SET_NODELOC(@1); $$ = int_const(inttable.add_string($1->get_string())); }
    | BOOL_CONST
      { @$ = @1; SET_NODELOC(@1); $$ = bool_const($1); }
    | STR_CONST
      { @$ = @1; SET_NODELOC(@1); $$ = string_const(stringtable.add_string($1->get_string())); }
    | OBJECTID 
