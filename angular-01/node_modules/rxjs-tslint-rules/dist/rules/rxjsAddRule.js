            ...
            if (this.normalizeFile(this.sourceFilePath) === this.normalizeFile(walker.sourceFilePath)) {
                if (!options.allowUnused) {
                    ...
                    Object.keys(addedObservables).forEach(function (key) {
                        if (!usedObservables_1[key]) {
                            addedObservables[key].forEach(function (node) { return _this.addFailureAtNode(tsutils.isImportDeclaration(node) ? node.moduleSpecifier : node, "Unused patched observable in " + options.file + ": " + key); });
                        }
                    });
                    Object.keys(addedOperators).forEach(function (key) {
                        if (!usedOperators_1[key]) {
                            addedOperators[key].forEach(function (node) { return _this.addFailureAtNode(tsutils.isImportDeclaration(node) ? node.moduleSpecifier : node, "Unused patched operator in " + options.file + ": " + key); });
                        }
                    });
                }
            }
                ...
                if (!options.allowElsewhere) {
                    Object.keys(this.addedObservables).forEach(function (key) {
                        _this.addedObservables[key].forEach(function (node) { return _this.addFailureAtNode(tsutils.isImportDeclaration(node) ? node.moduleSpecifier : node, "Patched observables are forbidden outside of " + options.file + ": " + key); });
                    });
                    Object.keys(this.addedOperators).forEach(function (key) {
                        _this.addedOperators[key].forEach(function (node) { return _this.addFailureAtNode(tsutils.isImportDeclaration(node) ? node.moduleSpecifier : node, "Patched operators are forbidden outside of " + options.file + ": " + key); });
                    });
                }
            }
        }
