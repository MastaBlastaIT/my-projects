...
const sortForm = formNode => {
  let formItems = Array.prototype.slice.call(
    formNode.querySelectorAll("input[name]")
  );
...
function sendRequest() {
  ...
  let message = [].reduce
    .call(
      payForm.childNodes,
      (res, node) => {
        return node.innerHTML !== undefined && node.hasAttribute("name")
  ...
  if (
    payForm.action.match(
      /^https:\/\/simpleapi\..*gateline\.net(\:18610|)\/pay$/
    )
  ) {
